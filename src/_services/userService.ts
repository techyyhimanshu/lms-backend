...
const studentsummary = async (userId: any): Promise<any> => {
    ...
    try {
        debugger;
        const usertype_list = await sequelize.query(`SELECT URNNO,NAME,SPONSOR,ENTRYDATE,EXPIRYDATE,TRAININGSTARTDATE,TRAININGENDDATE,COURSEID ,ORGSTRUCTURENAME COURSENAME,SEC_TO_TIME(TIMEPERIODMAX) TOTALTIME,SEC_TO_TIME(USEDTIME(100001)) USEDTIME,
...
const coursesummary = async (userId: any, cid: any): Promise<any> => {
    ...
    try {
       ...
       SELECT 
        ...
        SEC_TO_TIME(USEDTIME('${userId}')) AS usagetime,USEDTIME('${userId}') AS usedtimeinsecond,NODEKEY,GROUPTYPE        
    FROM ORGSTRUCTURE
    ...
    SELECT 
        ...
        SEC_TO_TIME(GetTotalTimeSecs('${userId}', c.ORGSTRUCTUREID)) AS usagetime,GetTotalTimeSecs('${userId}', c.ORGSTRUCTUREID) AS usedtimeinsecond, c.NODEKEY,c.GROUPTYPE
    FROM ORGSTRUCTURE c
